...
export async function juneBuild(
  ...
  // The deployed worker runs first (wrangler assets.run_worker_first) and wraps
  // the pipeline with withAssets — so prerendered pages, served from the ASSETS
  // binding, still get the Link header + Accept:markdown negotiation the asset
  // layer can't do. The Link header is frozen here from the same builder the
  // pipeline uses, so the static and dynamic surfaces advertise identically.
  const linkHeader = buildLinkHeader(frozen.agent);
  const entry = `// AUTO-GENERATED by \`june build\` — do not edit. Regenerate: june build .
import { withAssets } from "@junejs/server/worker";
${imports.join("\n")}
...
export default withAssets(pipeline, { link: ${JSON.stringify(linkHeader)} });
`;
      ...
      JSON.stringify(
        {
          ...
          // negotiation. The ASSETS binding lets the worker serve the static
          // files itself.
          ...(hasAssets
